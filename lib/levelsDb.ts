...
export const fetchAndSyncRemoteLevels = async (): Promise<void> => { // إزالة userId كمعامل
  ...
  try {
    ...
    if (!remoteLevels) {
      console.log('📭 لا توجد مستويات في Supabase');
      return;
    }
    ...
    await db.withTransactionAsync(async () => {
      // معالجة المستويات المحذوفة بعيدياً
      ...
      if (deletedLevels) {
        for (const deletedLevel of deletedLevels) {
          ...
          if (existingLocal && !existingLocal.deleted_at) {
            ...
            console.log(`🗑️ تم وضع علامة حذف على المستوى المحذوف بعيدياً: ${deletedLevel.name}`);
          }
        }
      }
      ...
      for (const remoteLevel of remoteLevels) {
        ...
        if (!localLevel) {
          ...
          console.log(`➕ تم إدراج مستوى جديد من Supabase: ${remoteLevel.name}`);
        } else {
          ...
          if (remoteUpdate > localUpdate && !localLevel.operation_type) {
            ...
            console.log(`🔄 تم تحديث المستوى المحلي من Supabase: ${localLevel.name}`);
          }
        }
      }
    });
